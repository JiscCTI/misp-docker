...
MISP can use OIDC to authenticate users, below are instruction on setting this up.

## 1 - Identity Provider Configuration

To setup MISP you need to have created MISP as a client in your OIDC Identity Provider.

By default, MISP will use JWTs signed with the Client Secret as the Client Authenticator with a
Challenge Mode of S256.

This will depend on which Identity Provider you are using and local policies, however some guidance
...
Accessing `https://{FQDN}[:{HTTPS_PORT}]` should redirect to authenticate against the Identity
Provider before being redirected back to MISP.

## Guidance for Configuring Identity Providers

### Keycloak

**Note:** You must be a realm admin in Keycloak in order to follow this guide. 

**Note:** This guide is based on the Keycloak admin console in version 23.
